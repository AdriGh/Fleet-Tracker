...
TRUCK_COLS = (3, 4, 9)
# Indices 1-based de las columnas por conductor (Pre-trip, Post-trip): se
# fusionan siempre con el nombre a lo largo de las filas del conductor.
DRIVER_COLS = (7, 8)
COL_WIDTHS = [16, 20, 12, 13, 12, 13, 13, 13, 13]
...
def _write_block(ws, start_row, date_label, groups):
    ...
    for group in groups:
        ...
        for entry in group["rows"]:
            for col, name in enumerate(COLUMNS, start=1):
                ...
                if name in BLUE_COLS:
                    ...
                    # Pre/Post-trip: '⚠ NO PRE-TRIP' en naranja (como NO DVIR);
                    # si no, rojo < 15 min y verde >= 15 min.
            ...
            # Pre-trip y Post-trip (G, H) quedan aparte con su propio estado.
        ...
        if row - group_start > 1:
            # Nombre (2) y Pre/Post-trip (7,8) son por conductor: se fusionan
