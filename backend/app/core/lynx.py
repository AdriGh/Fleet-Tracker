# -*- coding: utf-8 -*-
"""Ingesta + control de reefers vía Carrier Lynx Fleet API (OEM directo).

BOCETO H6 — espejo de `core/traccar.py` para la fuente OEM. A diferencia de
Traccar (aftermarket, solo lectura + umbral de alerta), el Lynx API es
**BIDIRECCIONAL**: lee la telemetría real del TRU (setpoint, supply/return/
ambient, modo, alarmas) Y permite **CONTROLARLO** (cambiar setpoint/modo,
defrost, power…) hablándole al APX Control System de la unidad. Confirmado
en el brochure Carrier *Lynx Fleet* 62-12176 Rev. C y la doc de integración:
*"two-way command APIs… can be integrated in your own systems"*.

El módulo Lynx viene de fábrica en los Carrier X4 2022 / Vector 8000; el
**dealer Carrier emite las credenciales** (Client ID + Client Secret + API
Key) al activar la suscripción. Auth = OAuth2 client-credentials.

SOBERANÍA DEL DATO (ver docs/STRATEGY-data.md): esta fuente es NUESTRA y
directa — no pasa por Samsara ni por ningún tercero.

TIER: el control remoto real requiere una suscripción ≥ *Monitor and
Control*. Con tier `monitor` (solo lectura) las funciones de control
devuelven un error claro en vez de pegarle al API. El tier se configura acá
hasta que el endpoint de cuenta del API lo exponga.

Config en `backend/lynx.local.json` (gitignored por `*.local.json`):
    {
      "base_url": "https://api.lynxfleet.carrier.com",  // CONFIRMAR (portal)
      "token_url": "",            // vacío => base_url + /oauth/token
      "client_id": "",            // del dealer Carrier
      "client_secret": "",        // del dealer Carrier
      "api_key": "",              // del dealer Carrier (header x-api-key)
      ...
      "deviation_f": 8.0,         // °F de desvío que dispara alarma derivada
      "stale_minutes": 20         // sin reportar => alarma "no data"
    }

PENDIENTE (no es público; pedir al dealer / portal `dev1.lynx.carrier.com`,
preguntas en docs/reefer-dealer-questions.md): los PATHS exactos de los
endpoints y los nombres de campos del JSON. Acá van como defaults
overrideables por config (`path_assets`/`path_command`/`path_history`) y los
campos se leen por `_pick()` con los nombres más probables, todo
centralizado para ajustarlo en un solo lugar cuando lleguen las credenciales.
"""
...
# Paths del API (CONFIRMAR con el portal Lynx; overrideables por config).
_PATH_ASSETS = "/api/v1/assets"                  # lista de TRUs + último estado
...
def _norm_tier(value) -> str:
    """'Monitor and Control' / 'enhanced' / etc. -> monitor|control|enhanced."""
...
def can_control(s: dict | None = None) -> bool:
    """¿La suscripción habilita control remoto real (≥ Monitor and Control)?"""
    s = s or load_settings()
...
def _alarms(asset: dict) -> list[dict]:
    """Alarmas reales del OEM -> forma normalizada del Cold Chain."""
    raw = _pick(asset, "alarms", "activeAlarms", "faults") or []
    out: list[dict] = []
    for a in raw if isinstance(raw, list) else []:
        if not isinstance(a, dict):
            continue
        out.append({
            "code": _pick(a, "alarmCode", "code", "id") or "",
...
def _build_unit(asset: dict, s: dict) -> dict:
    """Asset del Lynx API -> forma ReeferUnit del Cold Chain.

    NOMBRES DE CAMPO: el JSON exacto del Lynx API no es público; los accesos
    van por `_pick()` con los nombres más probables y deben confirmarse
    contra el schema del portal (un solo lugar). A diferencia de Traccar,
    acá setpoint/supply/ambient/modo/alarmas son REALES (OEM)."""
    aid = _pick(asset, "id", "assetId", "unitId", "deviceId")
    ...
    return_f = _temp_f(
        ...
        tu)
    supply_f = _temp_f(
        _pick(asset, "supplyAirTemperature", "supplyAir"), tu)
    ambient_f = _temp_f(
...
# ----- API públicas (forma idéntica a traccar.py) ------------------------

async def load() -> dict:
    """Reefers reales desde Lynx. {available, units, source, error}."""
    s = load_settings()
    if not is_configured(s):
        return {"available": False, "units": [], "source": "lynx",
    ...
    try:
        ...
        msg = ("Invalid Lynx credentials" if code in (401, 403)
               else f"Lynx HTTP {code}")
        return {"available": False, "units": [], "source": "lynx",
                "error": msg}
    except httpx.HTTPError as exc:
        return {"available": False, "units": [], "source": "lynx",
                "error": f"Could not reach Lynx: {type(exc).__name__}"}

    units = [_build_unit(a, s) for a in _as_list(data) if isinstance(a, dict)]
    units.sort(key=lambda u: (len(u["alarms"]) == 0, u["unit"]))
    return {"available": True, "units": units, "source": "lynx", "error": ""}


async def history(unit_id: str, hours: int = 24) -> dict:
    """Serie de temperatura de una unidad (id 'lynx-<assetId>')."""
    s = load_settings()
    if not is_configured(s):
        return {"unit_id": unit_id, "demo": False, "points": []}
    aid = unit_id.replace("lynx-", "", 1)
    ...
    points = [{
        ...
        "return_f": _temp_f(_pick(row, "returnAirTemperature", "returnAir",
                                  "boxTemperature"), tu),
        "supply_f": _temp_f(_pick(row, "supplyAirTemperature",
                                  "supplyAir"), tu),
...
async def _command(unit_id: str, command: str,
    ...
    """Envía un comando two-way al TRU vía el Lynx command API.

    Gateado por tier: sin ≥ Monitor and Control devuelve error SIN pegarle
    al API. Solo unidades 'lynx-' (una sonda aftermarket no controla)."""
    s = load_settings()
    if not is_configured(s):
        return {"ok": False, "detail": "Lynx not configured"}
    if not can_control(s):
        return {"ok": False, "detail": (
            "Remote control requires a Monitor and Control (or Enhanced) "
            "Lynx subscription tier")}
    if not unit_id.startswith("lynx-"):
        return {"ok": False,
                "detail": "Remote control is only available on Carrier Lynx units"}
    aid = unit_id.replace("lynx-", "", 1)
    ...
    try:
        ...
        code = exc.response.status_code
        return {"ok": False, "detail": (
            "Invalid Lynx credentials" if code in (401, 403)
            else f"Lynx HTTP {code}")}
    except httpx.HTTPError as exc:
        return {"ok": False,
                "detail": f"Could not reach Lynx: {type(exc).__name__}"}
    return {"ok": True, "detail": f"Command '{command}' sent to {unit_id}"}


async def set_setpoint(unit_id: str, setpoint_f: float) -> dict:
    """Cambia el setpoint REAL del reefer (no un umbral): requiere tier control."""
