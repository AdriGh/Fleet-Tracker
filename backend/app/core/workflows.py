...
def ensure_default() -> None:
    ...
    with SessionLocal() as s:
        # EXISTS por entidad, no count() de columnas: el filtro multi-tenant
        # (with_loader_criteria de OrgScoped) aplica a selects de entidad; un
        # count() lo esquivaría y contaría workflows de OTROS tenants — un
        # tenant nuevo jamás sembraría su Pre-trip (mismo porqué que en
        # setup_status.py).
        if s.scalars(select(Workflow).limit(1)).first() is not None:
            return
